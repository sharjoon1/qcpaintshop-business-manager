 ...
 * GET /api/roles
 * List all roles
 * Query params: user_type (staff/customer), status (active/inactive)
 */
...
/**
 * DELETE /api/roles/:id
 * Delete role (only if not system role and not assigned to anyone)
 */
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Check if role is assigned to any users (staff or customers)
    // Note: Will need to check users table when it's implemented
    // For now, just delete
 ...
 */
router.get('/permissions/by-module', async (req, res) => {
  try {
    const [permissions] = await pool.query('SELECT * FROM permissions ORDER BY module, action');
 ...
 */
router.delete('/:id/permissions/:permission_id', async (req, res) => {
  try {
