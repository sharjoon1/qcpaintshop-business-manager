    ...
    try {
        ...
        const [totals] = await pool.query(`
            SELECT
                ...
                COALESCE(SUM(CASE WHEN due_date < CURDATE() AND balance > 0 THEN balance ELSE 0 END), 0) as overdue_amount,
                COUNT(CASE WHEN due_date < CURDATE() AND balance > 0 THEN 1 END) as overdue_count,
                COUNT(CASE WHEN balance > 0 THEN 1 END) as outstanding_count,
                COALESCE(AVG(CASE WHEN due_date < CURDATE() AND balance > 0 THEN DATEDIFF(CURDATE(), due_date) END), 0) as avg_days_overdue
            FROM zoho_invoices
            WHERE balance > 0 AND status NOT IN ('void', 'draft')
        `);

        // Collection rate (30 days): payments collected / total outstanding at start of period
...
router.post('/remind', perm, async (req, res) => {
    try {
        ...
        for (const r of reminders) {
            ...
            // 1. Insert into whatsapp_followups queue
            ...
            `, [
                ...
                r.message_body, wfResult.insertId, req.user.id
            ]);
...
router.post('/remind/log', perm, async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`
            INSERT INTO collection_reminders (
                zoho_invoice_id, zoho_customer_id, customer_name, phone,
                reminder_type, message_content, status, sent_at, sent_by, notes
            ) VALUES (?, ?, ?, ?, ?, ?, 'sent', NOW(), ?, ?)
        `, [
            zoho_invoice_id || '', zoho_customer_id, customer_name || '', phone || '',
            reminder_type, notes || '', req.user.id, notes || ''
        ]);
    ...
    try {
        ...
        const [result] = await pool.query(`
            INSERT INTO payment_promises (
                ...
                promise_date, promise_amount, notes, follow_up_date, created_by
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `, [
            zoho_invoice_id || null, zoho_customer_id, customer_name || '',
            promise_date, promise_amount, notes || null, follow_up_date || null, req.user.id
        ]);
