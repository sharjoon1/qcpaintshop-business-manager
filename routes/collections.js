    ...
    try {
        ...
        const [customers] = await pool.query(`
            SELECT
                ...
                zcm.branch_id,
                zcm.credit_limit,
                COALESCE(zcm.zoho_unused_credits, 0) as unused_credits,
            ...
            ${where}
            GROUP BY zi.zoho_customer_id, zcm.zoho_contact_name, zcm.zoho_phone, zcm.branch_id, zcm.credit_limit, zcm.zoho_outstanding, zcm.zoho_unused_credits
...
// GET /pay-history — list payment links with status
router.get('/pay-history', requirePermission('collections', 'view'), async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM payment_links ORDER BY created_at DESC LIMIT 100');
