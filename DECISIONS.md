...
| 2026-06-12 | Vendor PO/bill financial model | **Line price = DPL (ex-GST cost per pack), qty = number of packs (nos).** Bill totals: subtotal = Σ(qty×DPL) → minus a single bill-level total discount (sum of all the bill's discounts) → taxable → **GST auto 18% (CGST9+SGST9), editable** when a bill differs → grand total must match the printed invoice. Verified on a real Berger bill (18700 − 1823.25 = 16876.75 → +18% = 19914.57). | vendor PO/bills |
| 2026-06-12 | Vendor bill DPL update | **Per-line, like HSN:** when a bill's DPL ≠ the stored item DPL, staff can update the Zoho item's DPL to the bill value from the reconciliation → writes `zoho_cf_dpl` + recomputes sales `zoho_rate` (`ceil(DPL×1.18×1.10)`) + pushes to Zoho (existing DPL machinery). | vendor bills |
...
| 2026-06-12 | Vendor-bill discount "applied but invisible" | Root cause was display + extraction, not the money model. Fixed: (1) `normalizeScan` deterministically RECOVERS a missing bill discount as `subtotal − (total − tax)` when the AI reads GST+totals but no discount; (2) verify-seeding applies discount **and** tax independently (old gate only seeded tax when discount was also absent → GST-but-no-discount); (3) the read-only bill view now shows the Discount + Taxable lines; (4) any non-pushed bill can be re-verified against its photo before push. | vendor bills |
| 2026-06-12 | Zoho location/branch on push | A location/branch picker is offered at **PO-create** (stored on the PO, carried to the converted bill) and at **bill push** + **invoice push**; the chosen `zoho_location_id` is sent to Zoho (`location_id`) and stamped on the row. Locations come from `zoho_locations_map` (synced from Zoho); pickers default to the value already on the document. New cols on vendor_purchase_orders/vendor_bills/billing_invoices (migration `20260612_zoho_location_salesperson.js`). | vendor + sales push |
| 2026-06-12 | Mandatory salesperson on invoice push | Zoho makes salesperson required → the sales-invoice push now **requires** a salesperson (throws `SALESPERSON_REQUIRED`); reuses the painter-program salesperson concept — **painter invoices default to the painter's mapped `zoho_salesperson_id`**, others pick from a new `zoho_salespersons` master (synced from Zoho `/settings/salespersons`). `salesperson_id` is sent to `createInvoice` and stamped on `billing_invoices`. | sales invoices |
