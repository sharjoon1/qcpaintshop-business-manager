...
### 2026-06-11 - Phase 4 Structural (A8/A12)
1. **A8: router splits** — `routes/painters.js` → `routes/painters/{public,painter,admin,middleware,shared,index}.js` (7/69/81 routes by auth domain); `routes/zoho.js` → `routes/zoho/{sync-config,invoices-reports,items,reorder,shared,index}.js` (28/18/47/55 by section). Pure mechanical moves; `index.js` re-exports the exact old module API, so `require('./routes/painters')`/`('./routes/zoho')` and server.js are untouched. Module state singletons (zoho `_apiCache` LRU, `_syncDebounce`, painter pool/io fan-out) live once in each `shared.js`. **Merge gate: `scripts/check-route-shadowing.js`** — proves the route SET is identical and no overlapping pair (e.g. `/me` vs `/:id`) changed relative match order; run it for any future route-file split.
2. **A12: OpenAPI spec** — `scripts/generate-openapi.js` → `docs/openapi.json` (1,027 operations, x-auth/x-permission per route, `--check` staleness mode; resolves split `routes/<name>/` directories through the index.js composition). Regenerate after route changes.
3. Review-caught fixes: reorder PDF `__dirname` depth (was writing into `routes/uploads/`), `app-metadata-collector` + `error-prevention-service` route scans made recursive (AI Analyzer would have lost the 306 split routes).
