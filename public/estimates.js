...
async function loadEstimates() {
    ...
    try {
        ...
        console.log('✅ Estimates loaded:', allEstimates.length);
...
function showActions(id) {
    ...
    const actions = [
        ...
        { label: '📋 Duplicate Estimate', action: 'duplicateEstimate' },
        { label: '🗑️ Delete Estimate', action: 'deleteEstimate', danger: true }
...
async function changeStatus(id, newStatus) {
    ...
    try {
        ...
        if (response.ok) {
            alert(`✅ Estimate marked as ${statusNames[newStatus]}!`);
...
async function duplicateEstimate(id) {
    ...
    try {
        ...
        if (!original) {
            alert('❌ Failed to load original estimate');
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Estimates page initialized');
